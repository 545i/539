...
def get_recover_exclude(user: str = Depends(current_user)):
    """回這個帳號在「建議車數/支數」裡排除的紀錄 id 清單(跨裝置)。"""
    return {"ids": recover_exclude_store.get_ids(user)}


@router.put("/recover-exclude")
