...
def add_entry(body: EntryIn, user: str = Depends(current_user)):
    ...
    return ledger_store.add_entry(user, body.mode, body.record)


@router.delete("/{entry_id}")
def delete_entry(entry_id: int, user: str = Depends(current_user)):
    """刪一筆(撤銷上一筆);不是自己的紀錄回 404。"""
    if not ledger_store.delete_entry(user, entry_id):
...
def clear_entries(mode: str | None = Query(default=None),
    ...
    if mode is not None:
        _check_mode(mode)
    return {"ok": True, "deleted": ledger_store.clear(user, mode)}
