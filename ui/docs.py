...
def render() -> None:
    ...
    st.markdown(
        "**拖牌包車**:拖 1 個膽號配其餘號碼 = 1 車"
        "(今彩539 / 天天樂 38 注;六合彩 48 注)。整車「中」的條件是**膽號被開出**:"
    )
    st.latex(
        r"P_{\text{膽中}} = \frac{\text{每期開出顆數}}{\text{號碼上限}}"
        r"\quad\Rightarrow\quad \frac{5}{39}\approx 12.82\%\ \text{(539/天天樂)},\ "
        r"\frac{6}{49}\approx 12.24\%\ \text{(六合彩)}"
    )
    st.caption(
        "膽號若在開出的號碼內,其餘開獎號各與膽號組成一個中獎注"
        "(539 恰 4 注中、六合彩恰 5 注中)。"
    )
    ...
    st.latex(r"\text{中獎金額}_{\text{平}} = \frac{\text{每車成本}}{P_{\text{膽中}}}")
    st.caption(
        "例(今彩539):每車成本 2755、中獎可得 21200 → 報酬率 = (5/39 × 21200)/2755 − 1 "
        "≈ −1.34%;損益兩平中獎金額 = 2755 × 39/5 = 21,489。"
    )
    st.caption(
        "例(六合彩):每車成本 3528、中獎可得 28500 → 報酬率 = (6/49 × 28500)/3528 − 1 "
        "≈ −1.08%;損益兩平中獎金額 = 3528 × 49/6 = 28,812。"
    )
    ...
    st.markdown(
        "**責任分母 $m$(嚴格 vs 平攤)**:上面要求「**任一款**中 1 顆就把 $L+T$ 全部拿回」,"
        "這是最嚴格的一種。把它放寬成「每款只負責 $\\frac{1}{m}$」,同一套推導可以一次涵蓋:"
    )
    st.latex(r"x_i\,w_i \ge \frac{L+T}{m} \quad\Rightarrow\quad T = (L+T)\frac{k}{m}"
             r"\quad\Rightarrow\quad L+T = \frac{m\,L}{m-k}, \qquad "
             r"\boxed{\;x_i = \frac{L}{(m-k)\,w_i}\;}\quad (k < m)")
    st.markdown(
        "- $m=1$ **嚴格**:任何一款中 1 顆就完全回本,但需要 $k<1$,成本最高。\n"
        "- $m=N$(款數)**平攤**:每款只負擔 $1/N$,條件放寬成 $k<N$,成本低很多,"
        "但**要 $N$ 款都中 1 顆才完全回本**;只中一款反而會比原本更虧。\n"
        "- 只下一款時 $N=1$,兩者是同一條式子,也就是畫面上的「單押回本」 "
        "$x=\\lceil L/(w-nc)\\rceil$。"
    )
    st.caption(
        "例(虧 135,570、三款各押 5 顆):嚴格 k=1.92 ≥ 1 無解;平攤 m=3 有解,"
        "總成本 281,050,三款都中 1 顆 → +22,680,但只中一款 → −274,120。"
        "工具會把這兩個數字與「全中的機率」一起顯示,不會只報好消息。"
    )


    st.error(
        "**常見誤解**:「只打這一款要幾車」($\\lceil L/(w-nc)\\rceil$)**不能拿來多款一起下**。"
        "兩款各自照單款算出來的車數去下,成本會付兩份、中獎卻只收得到一份 —— "
        "例:虧 127,305 時,天天樂押 5 顆單獨要 18 車、六合彩押 2 顆單獨要 6 車;"
        "兩款都下的話總成本 290,286,天天樂中 1 顆只回到 −35,991,還是虧。"
        "正解是把兩款一起解:59 車 + 44 車,中任一款 1 顆才真的回本。"
        "所以工具的表格只給「已把當天全部成本算進去」的建議車數,並在每一列標出中 1 顆後的累積。"
    )
    st.markdown(
    ...
    st.latex(r"k_V = \sum_{i \in V} \frac{n_i c_i}{w_i}, \qquad "
             r"L + T = \frac{m\,(L + T_F)}{m - k_V}, \qquad "
             r"x_i = \frac{L + T_F}{(m-k_V)\,w_i}\ \ (i \in V)")
    st.caption(
    ...
    # ── 八、多遊戲與預估開機率 ──────────────────────────
    st.subheader("八、多遊戲與預估開機率(core/games.py / core/picker.py)")
    st.markdown(
        "今彩539 與 天天樂(加州 Fantasy 5)**玩法都是 39 選 5**,組合數與機率(第一節)完全相同,"
        "差別只在票價與獎金結構;六合彩則是 **49 選 6**,連組合數都不同:"
    )
    st.latex(
        r"\binom{39}{5} = 575{,}757 \qquad \binom{49}{6} = 13{,}983{,}816"
    )
    ...
    st.caption(
        ...
        "六合彩為 pari-mutuel 浮動彩池,頁面上的數字是歷史平均粗估。各遊戲資料與統計完全獨立。"
    )
    st.markdown(
        "**預估開機率**:把各策略權重 $w(n)$ 正規化,使總和為每期開出的號碼數"
        "(539/天天樂 5、六合彩 6):"
    )
    st.latex(r"\hat{p}(n) = \text{每期開出顆數} \times \frac{w(n)}{\sum_{m} w(m)}")
    st.caption(
        "random 時 539/天天樂 每號 5/39 ≈ 12.8%、六合彩每號 6/49 ≈ 12.2%(也是真實機率)。"
        "非 random 只是把歷史傾向視覺化,**沒有預測下一期的能力** —— 每期獨立隨機,"
        "真實開機率永遠均勻。"
    )
