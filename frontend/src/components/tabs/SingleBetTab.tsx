  ...
  return (
    <div className="space-y-4 sm:space-y-5 animate-in fade-in duration-200 w-full overflow-hidden">
      ...
      <div className="p-4 sm:p-5 rounded-2xl bg-white dark:bg-[#121212] border border-black/[0.08] dark:border-white/[0.08] flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-4">
        ...
        <div className="flex items-center justify-between md:justify-end gap-4 border-t md:border-t-0 pt-2.5 md:pt-0 border-black/[0.06] dark:border-white/[0.06]">
          <div className="text-left md:text-right">
            <span className="text-[10px] uppercase tracking-wider text-neutral-400 block">單顆累積損益</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-5">
          ...
          <div className="p-4 sm:p-5 rounded-2xl bg-white dark:bg-[#121212] border border-black/[0.08] dark:border-white/[0.08] space-y-4">
            ...
            <div>
              <LotteryBallPad
                ...
                onClear={() => setSelectedBall([])}
                onQuickSelect={(balls) => setSelectedBall(balls.slice(0, 1))}
                maxBalls={1}
                totalBalls={numMax}
                label="鎖定下注號碼 (單選 1 顆)"
              />
            ...
            {/* Car / Multiplier Stepper for Mobile Fast Operation */}
            <div className="space-y-2 pt-1">
              ...
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => setCars(Math.max(0.5, Number((cars - (cars <= 1 ? 0.5 : 1)).toFixed(1))))}
                ...
                <button
                  type="button"
                  onClick={() => setCars(Number((cars + (cars < 1 ? 0.5 : 1)).toFixed(1)))}
                  className="w-10 h-10 rounded-xl border border-black/10 dark:border-white/10 flex items-center justify-center text-neutral-700 dark:text-neutral-200 hover:bg-black/5 dark:hover:bg-white/5 active:scale-95 transition-all"
                >
            ...
            {/* Live Financial HUD Preview Card */}
            <div className="p-3.5 sm:p-4 rounded-xl bg-black/[0.02] dark:bg-white/[0.03] border border-black/[0.06] dark:border-white/[0.06] space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                ...
                <div>
                  <span className="text-neutral-500 block text-[10px]">命中預估彩金:</span>
                  <span className="font-mono font-bold text-sm text-emerald-600 dark:text-emerald-400">
                    NT$ {potentialPrize.toLocaleString()}
                  </span>
                </div>
                <div>
                  <span className="text-neutral-500 block text-[10px]">命中淨回報:</span>
                  <span className="font-mono font-bold text-sm text-emerald-600 dark:text-emerald-400">
                    +{potentialNet.toLocaleString()}
                  </span>
                </div>
                <div>
                  <span className="text-neutral-500 block text-[10px]">選定號碼:</span>
                  <span className="font-mono font-bold text-sm text-neutral-900 dark:text-white">
                    {selectedBall.length > 0 ? selectedBall[0].toString().padStart(2, '0') : '未選擇'}
                  </span>
                </div>
              </div>
            </div>

            {/* Action Buttons Panel - Full Width on Mobile */}
            <div className="pt-2 flex flex-col sm:flex-row gap-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  type="button"
                  onClick={() => handleRecord('中了 1 顆 (中獎)')}
        ...
        {/* Right Column (7/12) - Metrics, Recovery HUD & Live Ledger */}
        <div className="lg:col-span-7 space-y-4">
          ...
          {/* Records Ledger Section - Dual View: Cards on Mobile, Table on Desktop */}
                  ...
                  <div className="flex items-center justify-between text-[11px] pt-1 border-t border-black/[0.04] dark:border-white/[0.04]">
                    <div className="flex items-center gap-1">
                      ...
                      {rec.selectedBalls.map(b => (
                        <span key={b} className={`px-1.5 py-0.2 rounded font-mono text-xs ${rec.drawBalls.includes(b) ? 'bg-emerald-600 text-white font-bold' : 'bg-black/5 dark:bg-white/10'}`}>
                          {b.toString().padStart(2, '0')}
                        </span>
                      ))}
                    </div>
                    <div className="flex items-center gap-1">
                      <span className="text-neutral-400 text-[10px]">開獎:</span>
                  ...
                  {records.map((rec) => (
                    <tr key={rec.id}>
                      ...
                      <td className="font-mono text-xs">
                        {rec.selectedBalls.map(b => (
                          <span key={b} className={`px-1.5 py-0.5 rounded ${rec.drawBalls.includes(b) ? 'bg-black text-white dark:bg-white dark:text-black font-bold' : ''}`}>
                            {b.toString().padStart(2, '0')}
                          </span>
                        ))}
                      </td>
