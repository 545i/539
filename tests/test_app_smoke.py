...
def test_combo_records_a_round_with_the_numbers(app):
    ...
    assert r["numbers"] == 56 and r["cars"] == 1        # C(8,3)
...
def test_combo_autosettles_without_any_button(app):
    ...
    storage.add_round("apptest", "lotto539", date, 56, 1, None, 3528.0, 57000.0,
    ...
    assert r["hits"] == 5
    assert r["payout"] == 5 * 57_000 and r["net"] == 5 * 57_000 - 3_528
...
def test_combo_settles_by_issue_not_by_date(app):
    ...
    storage.add_round("apptest", "fantasy5", d_old, 56, 1, None, 3528.0, 57000.0,
