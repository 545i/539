...
# 而且組頭本來就是直接報「中一碰給你多少」。
MARKET_PRIZE: dict[int, float] = {2: 72.5 * 53, 3: 57_000.0, 4: 750_000.0}
...
def star_bets(stars: int, picked: int = STAR_PICK) -> int:
    ...
    選 8 顆:三星 56 × 5 = 280、四星 70 × 4 = 280。

    使用者一開始說單支成本是 63 × 56 = 3,528,但那撐不住 —— 用他自己的
    819 期開獎回測,8 顆重 3 顆的機率是 4.88%(與理論 4.91% 吻合),
    配上中一次 5 × 57,000 = 285,000,返還率會是 394%,組頭早就收攤了。
    乘上碰數(× 5 / × 4)後是 79% / 83%,兩種星別同時落在合理區間 ——
    所以 63 × 56 是**一個「碰組」**的價,下 8 顆三星是買 5 組。
    (真的只付 3,528 的話,UI 上碰數欄位可以直接改回 56。)
    """
